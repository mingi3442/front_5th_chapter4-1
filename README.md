...
Artillery를 통한 체계적인 부하테스트를 실시하여 총 **14,850개의 요청**을 처리했습니다.
...
| 테스트 위치 | 대상 서비스 | 평균 응답시간 | 완료된 요청수 | 성공률 | 비고         |
| ----------- | ----------- | ------------- | ------------- | ------ | ------------ |
| **한국**    | 한국 S3     | 27ms          | 14,888        | 99.99% | 지역 내 접근 |
| **한국**    | 미국 S3     | 27ms          | 14,883        | 99.99% | 대륙간 접근  |
| **한국**    | CloudFront  | 27ms          | 14,888        | 99.99% | CDN 캐싱     |
| **미국**    | 한국 S3     | 47ms          | 14,808        | 100%   | 대륙간 접근  |
| **미국**    | 미국 S3     | 47ms          | 14,808        | 98%    | 지역 내 접근 |
| **미국**    | CloudFront  | 46ms          | 14,808        | 100%   | CDN 캐싱     |

![alt text](./assets/cf.png)

![alt text](./assets/korea-s3.png)

![alt text](./assets/usa-s3.png)
...
| ---------- | ------------ | --------- | ----------- |
| 한국 S3    | 27           | 99.99     | 14,888      |
| 미국 S3    | 27           | 99.99     | 14,883      |
| CloudFront | 27           | 99.99     | 14,888      |
...
- **한국에서 접근**: 모든 서비스가 99.99% 이상의 높은 성공률
- **미국에서 접근**: CloudFront가 100% 성공률로 **미국 S3 직접 접근(98%) 대비 2% 개선**
...
**📸 [이미지 삽입 위치: S3 네트워크 탭 스크린샷 2장]**

- S3 접근 (캐시 지우기 후)
  ![s3-no-cache](./assets/s3-no-cache.png)
...
**📸 [이미지 삽입 위치: CloudFront 네트워크 탭 스크린샷 2장]**

- CloudFront 접근 (캐시 지우기 후)
  ![cf-no-cache](./assets/cf-no-cache.png)
...
1. **안정성 향상**: 특히 미국 S3 직접 접근 시 98% 성공률을 CloudFront를 통해 100%로 개선
2. **글로벌 일관성**: 전 세계 어디서든 비슷한 성능 보장
3. **브라우저 경험 개선**: 페이지 로딩 시간의 실질적 단축 (39.8% 개선)
4. **장애 대응**: 오리진 서버 장애 시 다른 엣지 로케이션에서 서비스 계속 제공
